...
public class ChinaMoblie_netsplit {
...
//		editText.get(4).sendKeys(casephone2);
	...
	public void clearGroup() {
		...
		deleteAllContacts();
		if(m>17){
			Myassert("clear group error." + this.getTestCaseName(), false);
		}
	...
	public void setValue(String id, String time) {
		...
		if(i>61){
			Myassert("getvalue error" + this.getTestCaseName(), false);
		}
...
//		intoContentEditTextByName(msscnt, "");
	...
	public void startTestCase() {
		...
		Myassert(mothodName + "当前运行程序不是和通讯录: " + curactivity , curactivity.equals(appActivity), mothodName);
		
		Myassert("不在和通讯录主页1" + mothodName, isExistenceById("iab_title"));
		Myassert("不在和通讯录主页2" + mothodName, getTextViewNameById("iab_title").equals("和通讯录"));
	}
	...
	public void Myassert(boolean bl, String caseName) {
		if (bl == false) {
			ChinaMoblie_netsplit.snapshot(driver, caseName);
		}
		sleepTime(3000);
		Assert.assertTrue(bl);
	}
	...
	public void Myassert(String message, boolean bl) {
		if (bl == false) {
			ChinaMoblie_netsplit.snapshot(driver, getMethodName());
		}
		sleepTime(3000);
		Assert.assertTrue(message, bl);
	}
